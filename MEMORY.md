...
### 2026-05-14 摘要 （重要鉄律補充！）
- 子 Agent spawn 时必须指定 `agentId`，否则跑的是阿悄自己的配置，相当于子Agent根本没启用。
- 已写入：`clawbox-installer-v3` 项目构建中发现的 Agent 调用问题。
...
- 新增铁律/SOP 变更：
  ...
  - #12 任务即上下文。
  - #13 SSH/exec 由阿悄直做。
  - #14 sessions_spawn 必须指定 agentId，否则默认跑自己（阿悄）的配置。
